...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      >
        <span className="inline-block rounded-full border border-border/70 bg-foreground px-4 py-1.5 text-xs tracking-widest text-background uppercase backdrop-blur-sm" style={{ fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace' }}>
          {portfolioData.hero.role}
        </span>
        <h1 className="mx-auto max-w-3xl text-4xl font-bold tracking-tight sm:text-5xl lg:text-6xl">
          {portfolioData.hero.fullName}
        </h1>
        <p className="flex flex-wrap items-baseline justify-center gap-x-2 text-lg text-muted-foreground sm:text-xl" style={{ fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace' }}>
          I build{" "}
          <TextType
            as="span"
            text={["Web Apps.", "Mobile Apps.", "Cloud Solutions.", "Full-Stack Systems."]}
            typingSpeed={75}
            deletingSpeed={50}
            pauseDuration={1500}
            cursorCharacter="|"
        ...
        <PdfViewerModal
          ...
          pdfUrl="/muhamad-izamuddin-resume.pdf"
        />
        ...
        <div className="flex items-center justify-center gap-3">
          {portfolioData.contact.socials.map((social) => {
            ...
            const color = brandColor[social.label];
            return (
              <a
                key={social.label}
                href={social.href}
                ...
                className="flex size-11 items-center justify-center rounded-2xl border border-border/70 bg-background text-foreground transition-all duration-200"
                onMouseEnter={(e) => {
                  if (!color) return;
